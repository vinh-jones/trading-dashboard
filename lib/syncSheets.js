...
export async function syncFromSheets(supabase) {
  ...
  if (nullStrikeRows.length > 0) {
    const { data: existingNullStrike } = await supabase
      .from("trades")
      .select("ticker, type, open_date, close_date, description")
      .is("strike", null)
      .is("contracts", null);

    const existingNullKeys = new Set(
      (existingNullStrike || []).map(
        t => `${t.ticker}|${t.type}|${t.open_date}|${t.close_date}|${t.description}`
      )
    );

    const newNullStrikeRows = nullStrikeRows.filter(
      t => !existingNullKeys.has(`${t.ticker}|${t.type}|${t.open_date}|${t.close_date}|${t.description}`)
    );
