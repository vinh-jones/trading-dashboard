...
export async function syncFromSheets(supabase) {
  ...
  if (nullStrikeRows.length > 0) {
    ...
    for (const t of nullStrikeRows) {
      ...
      const exactMatch = existing.find(r => r.close_date === t.close_date);
      if (exactMatch) continue; // already up to date
      // Either brand new OR close_date changed → delete stale, insert fresh
