...
// each such lot, distinguished from CSP-assigned lots by date+share-count
...
function buildDirectBuyShareTrades(openSharesByTicker, cspClosed) {
  ...
  // Build a set of (ticker | close_date | option-contract-count) keys to skip.
  const cspKeys = new Set(
    (cspClosed || [])
      .filter(t => t.type === "CSP" && t.subtype === "Assigned" && t.close_date)
      .map(t => `${t.ticker}|${t.close_date}|${t.contracts}`)
  );
  ...
  for (const [ticker, lots] of Object.entries(openSharesByTicker || {})) {
    for (const lot of lots) {
      ...
      const key = `${ticker}|${lot.open_date}|${cspContractEquivalent}`;
      if (cspKeys.has(key)) continue; // CSP-assigned lot — trade already exists
