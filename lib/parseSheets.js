...
function processCSP(rows) {
  ...
  for (const row of rows) {
    ...
    const delta        = parseNum(col(row, 11));  // Execution Delta
    const entryCost    = parseNum(col(row, 14));  // Premium per share (STO price)
    const exitCost     = parseNum(col(row, 15));  // Buy to close price
