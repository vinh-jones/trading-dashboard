  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        let query = supabase
          .from("uw_signals")
          .select("ticker, gamma_env, flow_sentiment, whale_put_sells, short_interest_pct, earnings_expected_move_pct, next_earnings_date, gex_env, gex_net_gamma, gex_support, gex_resistance, gex_air_pocket, gex_refreshed_at, refreshed_at");
